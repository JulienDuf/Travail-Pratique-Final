...
public:
	...
	CGame(string _strEmplacementMap, unsigned int _iNombre…quipe, unsigned int _iNombreJoueur, CVent* _pVent, void _MapDestruction(int _iRayon, int _iX, int _iY), SDL_Surface* _Rotation(SDL_Surface* _pSurfaceRotation, float _fAngle), SDL_Renderer* _pRenderer) {
		...
		// DÈbut du Chrono de la physique
	...
	~CGame() {
		...
		delete m_pTeamList; // Liste d'Èquipe.
	...
	// ProcÈdure permettant de changer de tour...
	// En entrÈe:
	...
	// ProcÈdure changeant l'Èquipe acitive.
	void ChangerTeamActive() {
		m_pTeamList->AllerSuivantCurseur();
	}

	// ProcÈdure permettant d'afficher la game...
	// En entrÈe:
	// Param1: Renderer d'affichage.
	// Param2: Si la game est en pause.
	void AfficherGame(SDL_Renderer* _pRenderer, bool _boPause) {

		// Si c'est le dÈbut de la partie ou qu'un tour vient de se terminer...
		...
		// Affichage des dommages infligÈs aux joueurs...
	...
	// ProcÈdure rÈagissant au ÈvÈnement du programme.
	// En entrÈe:
	// Param1: Le gestionnaire d'ÈvÈnement...
	void ReactToEvent(SDL_Event* _pEvent) {
		
		// …vÈnement par rapport ‡ la game...
		switch (_pEvent->type) {

		// Si c'est une touche d'enfoncÈe...
		case SDL_KEYDOWN:
		...
		// Initialisation d'un entier correspondant ‡ l'arme doubleclickÈe...
	...
	// ProcÈdure qui gËre la physique du joueur actif...
	// ParamËtre : _pRenderer, Rendeur de la fenÍtre de jeu.
	// Ne retourne rien.
		...
		if (!m_boDebutPartie) {
			...
			if (m_pTimerPhysique->IsDone()) {
				...
				// Si le player n'est pas stable...
					...
					// DÈtection de collision avec pack...
					DetectionCollisionPack(pPlayerActif, &boExplosion, &PointExplosion, _pRenderer);		// On vÈrifie si il y a une collision entre le joueur et un pack.
					...
					if (boExplosion) {									// Si il y a eu une explosion...
						...
						// On le fait bouger selon son vecteur de dÈplacement vecteur vitesse...
						...
						if (!VerifierCollisionJoueurMap(pPlayerActif, RectTmp, &boCorps, &boPied, &_uiXPieds, &_uiYPieds, &_uiXCorps, &_uiYCorps)) {			// Si il n'y a pas de collision...
							...
							// On fait bouger le joueur selon sa vitesse et la gravitÈ
							pPlayerActif->ObtenirVecteurPoids()->ModifierComposantY(pPlayerActif->ObtenirVecteurPoids()->ObtenirComposanteY() + m_pGameMap->ObtenirGravite()->ObtenirComposanteY());
							...
							// On vÈrifie si la distance est assez grande entre le joueur et le sol pour qu'il tombe en chute libre.
							int iH = 0;
							...
							if (boCorps) {			// Si il y a une collision avec le corps...
								...
								if (!boPied) {			// Si il n'y a pas de collision avec le pieds...
									...
									// On continue ‡ faire descendre le joueur selon la gravitÈ.
									pPlayerActif->ObtenirVecteurPoids()->ModifierComposantY(pPlayerActif->ObtenirVecteurPoids()->ObtenirComposanteY() + m_pGameMap->ObtenirGravite()->ObtenirComposanteY());
							...
							if (boPied) {		// Si il y a une collision avec le pieds...
								...
								if (!boCorps) {			// Si il n'y a pas de collision avec le corps...
									...
									// On vÈrifie si la pente est trop grande pour monter.
									unsigned int uiH = 0;
					...
					// Les glissades n'ont pas ÈtÈ finies ‡ temps pour la remise, malheureusement... :(  Ummm... C'est pas moi! Umm.... C'est Xavier! :|    -Nico
...
>>>>>>> 18f1ba177055ad43f71db31fb7c35764d516aad2
	...
	void PhysiqueTool(SDL_Renderer* _pRenderer) {
		...
		if (m_pToolBar->ObtenirPositionObjetDoubleClick() <= 2) {
			...
			if (!pProjectileTmp->ObtenirSprite("")->IsActif() && pProjectileTmp->ExplosionEnCours()) {
				...
				// On enlËve les joueurs qui sont mort durant le processus...
				m_pTeamList->AllerATrieur(0);
				...
				// Tous les joueurs sont affectÈ par la physique.
				m_boDebutPartie = true;
				m_boFinTour = true;
				m_pToolBar->NouveauTour();
			}
				...
				if (m_pToolBar->ObtenirPositionObjetDoubleClick() == 0 && pProjectileTmp->EstLancer())  {
					...
					if (CollisionMissile(pProjectileTmp->ObtenirSurface(), *pProjectileTmp->ObtenirRectDestination(), &iX, &iY, _pRenderer)) {
						...
						// RÈaction ‡ la collision et dommage...
					...
					// Si le missile sort de l'Ècran, sauf vers le haut...
	...
	// ParamËtre: _puiXPieds, pointeur d'entier non-signÈ qui indique la position en x de la collision dans le rectangle du joueur entre les pieds et la carte de jeu s'il y a lieu.
	...
	// ParamËtre: _puiYCorps, pointeur d'entier non-signÈ qui indique la position en y de la collision dans le rectangle du joueur entre le corps et la carte de jeu s'il y a lieu.
	...
	bool VerifierCollisionJoueurMap(CPlayer* _pPlayer, SDL_Rect _RectPlayer, bool* _pboCollisionCorps, bool* _pboCollisionPieds, unsigned int* _puiXPieds, unsigned int* _puiYPieds, unsigned int* _puiXCorps, unsigned int* _puiYCorps) {
		...
		if (_pPlayer->ObtenirSpriteCourse()->IsActif()) {						// Si le joueur est en train de courir...
			...
			if (_pPlayer->ObtenirSpriteSaut()->ObtenirEtage() == 0)					// Si le jueur est orientÈ vers la droite...
				TmpSDLRectPlayerHitboxCorps = _pPlayer->ObtenirHitboxCorpsDroite();		// On se sert du rectangle pour le corps de droite. 
			else																		// Sinon...
				TmpSDLRectPlayerHitboxCorps = _pPlayer->ObtenirHitboxCorpsGauche();		// On se sert du rectangle pour le corps de gauche.	
			...
			if (_pPlayer->ObtenirSpriteParachute()->ObtenirEtage() == 0)					// Si le jueur est orientÈ vers la droite...
				TmpSDLRectPlayerHitboxCorps = _pPlayer->ObtenirHitboxCorpsDroite();		// On se sert du rectangle pour le corps de droite. 
			else																		// Sinon...
				TmpSDLRectPlayerHitboxCorps = _pPlayer->ObtenirHitboxCorpsGauche();		// On se sert du rectangle pour le corps de gauche.	
			...
			if (_pPlayer->ObtenirSpriteRepos()->ObtenirEtage() == 0)					// Si le jueur est orientÈ vers la droite...
				TmpSDLRectPlayerHitboxCorps = _pPlayer->ObtenirHitboxCorpsDroite();		// On se sert du rectangle pour le corps de droite. 
			else																		// Sinon...
				TmpSDLRectPlayerHitboxCorps = _pPlayer->ObtenirHitboxCorpsGauche();		// On se sert du rectangle pour le corps de gauche.	
	...
	void DommageExplosion(SDL_Point _ExplosionPoint, unsigned int _uiRayon, SDL_Renderer* _pRenderer) {
		...
		for (int i = 0; i < m_pTeamList->ObtenirCompte() && m_pTeamList->ObtenirCompte() > 0; i++) {
			...
			// Pour tous les joueurs des Èquipes...
			...
			for (int j = 0; j < pPlayerList->ObtenirCompte() && pPlayerList->ObtenirCompte() > 0; j++) {
				...
				if (pPlayerTmp->GetHealth() > 0) {
					...
					// Si le joueur est complËtement dans l'explosion...
		...
		for (int i = 0; i < pPackList->ObtenirCompte(); i++) {
			...
			if (!pPackTmp->IsUse()) {
				...
				// Si le pack est complËtement dans l'explosion...
	...
	// ProcÈdure infligeant un dommage avec l'arme de mÍlÈe...
