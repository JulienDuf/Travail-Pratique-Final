...
public:
	...
	*/
	...
	// Procédure permettant de parcourir la liste et d'afficher tous les contrôles qu'elle contient...
	// En entrée:
	// Param1: Le renderer de destination du contrôle.
	void ParcoursShowControl(SDL_Renderer* _pSDLRenderer) {
		if (m_pDerniere != nullptr) {
			int i = 0;
			while (i < m_uiCompte) {
				m_pCurseur->ObtenirElement()->ShowControl(_pSDLRenderer);
				m_pCurseur = m_pCurseur->ObtenirSuivante();
				i++;
			}
			m_pCurseur = m_pCurseur->ObtenirSuivante();
